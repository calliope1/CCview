...
{
    public interface IRelationDatabaseOld<Card, Atom, Rel, Art, Thm, Mod, Sent, RelType>
        where Card : ICC
        where Atom : IAtomicRelation
        where Rel : IRelation<Rel>
        where Art : IArticle
        where Thm : ITheorem
        where Mod : IModel
        where Sent : ISentence
        where RelType : IRelationType
    {
        IReadOnlyDictionary<int, Card> GetCardinals();
        HashSet<Atom> GetAtomicRelations();
        IReadOnlyDictionary<int, Rel> GetRelations();
        //private Dictionary<Sentence, (Relation Oldest, Relation Shortest)> OldestAndShortestRelations { get; set; } = [];
        IReadOnlyDictionary<int, Art> GetArticles();
        IReadOnlyDictionary<int, Thm> GetTheorems();
        IReadOnlyDictionary<int, Mod> GetModels();
        ...
        int LogicTransClose();
        Rel AddCtoCRelation(Card? a, Card? b, char type, Thm? witness);
        Rel AddMCNRelation(Mod? m, Card? c, int n, char type, Thm? witness);
        ...
        Art AddArticle(Art article);
        Art AddArticle(string? name, int date, string? citation, int id);
        Art AddArticle(string? name, int date, string? citation)
        {
            int newId = NewDictId(GetArticles());
            return AddArticle(name, date, citation, newId);
        }
        Card AddCardinal(string? name, string? symbol, int id);
        Card AddCardinal(string? name, string? symbol, bool fast = false)
        {
            return AddCardinal(name, symbol, NewDictId(GetCardinals(), fast));
        }
        Thm AddTheorem(Art article, string description, HashSet<Sent> results, int id);
        Thm AddTheorem(Art article, string description, HashSet<Sent> results)
        {
            int newId = NewDictId(GetTheorems());
            return AddTheorem(article, description, results, newId);
        }
        static bool AddResultToTheorem(Thm theorem, RelType type, int[] ids)
        {
            return theorem.AddResult(type, ids);
        }
        Mod AddModel(Art article, string? description, int id);
        Mod AddModel(Art article, string description)
        {
            int newId = NewDictId(GetModels());
            return AddModel(article, description, newId);
        }
        HashSet<Rel> GetMinimalRelations(IReadOnlyDictionary<int, Card> desiredCardinals);
        ...
        Card? CardinalBySymbolOrNull(string symbol);
        Card GetCardinalBySymbol(string symbol);
        Card? CardinalByNameOrNull(string name);
        Card GetCardinalByName(string name);
        int GenerateAtoms();
        static abstract HashSet<Atom> GenerateAtoms(Thm theorem);
        /// <summary>
        ...
        Card? CardinalFromProperties(string property);
        /// <summary>
